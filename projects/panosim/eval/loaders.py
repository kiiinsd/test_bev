...
def load_gt(pano: PanoSim, eval_split: str, box_cls, verbose: bool = False) -> EvalBoxes:
    """
    Loads ground truth boxes from DB.
    :param pano: A PanoSim instance.
    ...
    if box_cls == DetectionBox:
        attribute_map = {a['token']: a['name'] for a in pano.attribute}

    if verbose:
        print('Loading annotations for {} split from nuScenes version: {}'.format(eval_split, pano.version))
    # Read out all sample_tokens in DB.
    sample_tokens_all = [s['token'] for s in pano.sample]
    ...
    if eval_split == 'test':
        ...
        assert len(pano.sample_annotation) > 0, \
            'Error: You are trying to evaluate on the test set but you do not have the annotations!'

    sample_tokens = []
    for sample_token in sample_tokens_all:
        scene_token = pano.get('sample', sample_token)['scene_token']
        scene_record = pano.get('scene', scene_token)
    ...
    for sample_token in tqdm.tqdm(sample_tokens, leave=verbose):
        ...
        sample = pano.get('sample', sample_token)
        sample_annotation_tokens = sample['anns']

        sample_boxes = []
        for sample_annotation_token in sample_annotation_tokens:

            sample_annotation = pano.get('sample_annotation', sample_annotation_token)
            if box_cls == DetectionBox:
                # Get label name in detection task and filter unused labels.
                detection_name = NameMapping[sample_annotation['category_name']]
                ...
                sample_boxes.append(
                    box_cls(
                        ...
                        velocity=sample_annotation['vel'][:2],
...
def add_center_dist(pano: PanoSim,
    ...
    for sample_token in eval_boxes.sample_tokens:
        sample_rec = pano.get('sample', sample_token)
        sd_record = pano.get('sample_data', sample_rec['data']['LIDAR_TOP'])
        pose_record = pano.get('ego_pose', sd_record['ego_pose_token'])
...
def filter_eval_boxes(pano: PanoSim,
                      eval_boxes: EvalBoxes,
    ...
    for ind, sample_token in enumerate(eval_boxes.sample_tokens):
        ...
        sample_anns = pano.get('sample', sample_token)['anns']
        bikerack_recs = [pano.get('sample_annotation', ann) for ann in sample_anns if
                         pano.get('sample_annotation', ann)['category_name'] == 'static_object.bicycle_rack']
