...
class DeepSort(object):
    ...
    def update(self, bbox_xywh, confidences, bbox_idx, ori_img):
        self.height, self.width = ori_img.shape[:2]
        # generate detections
        features = self._get_features(bbox_xywh, ori_img)  # 获取当前bbox的特征
        bbox_tlwh = self._xywh_to_tlwh(bbox_xywh)
        detections = [Detection(bbox_tlwh[i], conf, features[i], bbox_idx[i]) for i, conf in enumerate(
        ...
        for track in self.tracker.tracks:
            ...
            bbox_id = track.bbox_idx
            outputs.append(np.array([x1, y1, x2, y2, track_id, bbox_id], dtype=int))
        if len(outputs) > 0:
